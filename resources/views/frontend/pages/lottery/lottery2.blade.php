...
<!-- Mirrored from raksy.dyndns.org/wheel.html by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 24 Aug 2023 07:35:08 GMT -->
<head>
    <meta charset="UTF-8">
    <style type="text/css">
        body { padding: 0px;
...
<button class="" style="width: 150px;height: 20px;background: red;border: 1px solid black;margin-top: 50px;color: white;">Go Back To Home</button>

<script>

    let canvas = document.getElementById("canvas");

    let sections = @json($user);
        ...
        if (wheels === null) {
            ...
            for (let selected=0; selected<sections.length; selected++) {
                ...
                for (let i=0; i<sections.length; i++) {
                    ...
                    ctx.font = "bold " + r/sections.length*1 + "px serif";
    ...
    canvas.onmousedown = function() {
        if (!running) {
            spinTo(Math.random()*sections.length|0,4000);
        }
    };
